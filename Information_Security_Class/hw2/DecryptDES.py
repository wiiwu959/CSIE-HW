...
def s_box(block, part):
	row = '' + block[0] + block[5]
	row = int(row, 2)
	column = '' + block[1] + block[2] + block[3] + block[4]
	column = int(column, 2)
...
def f_function(block):
	...
	result = []
	for i in range (48):
		block[i] = str((int(block[i]) + int(key[i])) % 2)
	for i in range (8):
		smallblock = block[i * 6:(i + 1) * 6]
...
def key_shift(block, round):
	popping = block.pop(27)
	block.insert(0, popping)
	popping = block.pop(55)
	block.insert(28, popping)
	if round not in (2, 9, 16):
		popping = block.pop(27)
		block.insert(0, popping)
		popping = block.pop(55)
		block.insert(28, popping)
...
for i in range (16):
	key = permutation(PC_2, pass_key)
	plaintext_left, plaintext_right = feistel(plaintext_left, plaintext_right, key)
	pass_key = key_shift(pass_key, i + 2)

plaintext_left, plaintext_right = plaintext_right, plaintext_left	#swap
plaintext = permutation(IP_1, plaintext_left + plaintext_right)
